...
abstract contract Extsload is IExtsload {
    ...
    function extsload(StorageKey slot) external view returns (bytes32) {
    ...
    /// @inheritdoc IExtsload
    function extsload(StorageKey startSlot, uint256 nSlots) external view returns (bytes memory) {
        assembly ("memory-safe") {
    ...
    /// @inheritdoc IExtsload
    function extsload(StorageKey[] calldata slots) external view returns (bytes32[] memory) {
